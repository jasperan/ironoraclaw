#!/bin/bash
# IronOraClaw OCI Instance Setup Script
# Runs via cloud-init on first boot - fully unattended
set -euo pipefail
exec > >(tee -a /var/log/ironoraclaw-setup.log) 2>&1

echo "=== IronOraClaw setup started at $(date) ==="
...
# -- 4. Build IronOraClaw --
echo "--- Building IronOraClaw ---"
git clone https://github.com/jasperan/ironoraclaw.git /opt/ironoraclaw
cd /opt/ironoraclaw
...
cp target/release/ironoraclaw /usr/local/bin/ironoraclaw
chmod +x /usr/local/bin/ironoraclaw
ironoraclaw --version || true

# -- 5. Initialize config --
echo "--- Initializing config ---"
export HOME=/home/opc
CONFIG_DIR="/home/opc/.ironoraclaw"
...
if [ "$ORACLE_MODE" = "freepdb" ]; then
  ...
  # Create ironoraclaw user
  docker exec oracle-free sqlplus -S "sys/${ORACLE_PWD}@localhost:1521/FREEPDB1 as sysdba" <<SQL || true
WHENEVER SQLERROR CONTINUE
CREATE USER ironoraclaw IDENTIFIED BY "${ORACLE_PWD}"
  DEFAULT TABLESPACE users QUOTA UNLIMITED ON users;
GRANT CONNECT, RESOURCE, DB_DEVELOPER_ROLE TO ironoraclaw;
GRANT CREATE MINING MODEL TO ironoraclaw;
EXIT;
SQL
...
TOMLEOF
  ...
  if [ -n "$ADB_WALLET_BASE64" ]; then
    WALLET_DIR="/home/opc/.ironoraclaw/wallet"
    mkdir -p "$WALLET_DIR"
...
sudo -u opc bash -c "export ORACLE_HOME=/usr/lib/oracle/23/client64 && export LD_LIBRARY_PATH=${ORACLE_HOME}/lib && /usr/local/bin/ironoraclaw setup-oracle"

# -- 8. Run onboard --
echo "--- Running onboard ---"
sudo -u opc bash -c "export ORACLE_HOME=/usr/lib/oracle/23/client64 && export LD_LIBRARY_PATH=${ORACLE_HOME}/lib && /usr/local/bin/ironoraclaw onboard" <<< "n" || true

# -- 9. Install and start gateway systemd service --
echo "--- Installing gateway service ---"
cat > /etc/systemd/system/ironoraclaw-gateway.service <<'UNIT'
[Unit]
Description=IronOraClaw Gateway
...
ExecStart=/usr/local/bin/ironoraclaw gateway
Restart=on-failure
RestartSec=10
...
systemctl enable --now ironoraclaw-gateway

# -- 10. Done --
echo "=== IronOraClaw setup completed at $(date) ==="
touch /var/log/ironoraclaw-setup-complete
