...
use ironoraclaw::setup::{SetupConfig, SetupWizard};
...
async fn main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Config(config_cmd)) => {
            init_cli_tracing();
            return ironoraclaw::cli::run_config_command(config_cmd.clone()).await;
        }
        Some(Command::Registry(registry_cmd)) => {
            init_cli_tracing();
            return ironoraclaw::cli::run_registry_command(registry_cmd.clone()).await;
        }
        ...
        Some(Command::Doctor) => {
            ...
            ironoraclaw::bootstrap::load_ironclaw_env();
            return ironoraclaw::cli::run_doctor_command().await;
        }
        Some(Command::Status) => {
            init_cli_tracing();
            let _ = dotenvy::dotenv();
    ...
    let config = match Config::from_env_with_toml(toml_path).await {
        Ok(c) => c,
        Err(ironoraclaw::error::ConfigError::MissingRequired { key, hint }) => {
    ...
    let log_level_handle =
        ironoraclaw::channels::web::log_layer::init_tracing(Arc::clone(&log_broadcaster));
    ...
    if config.llm.backend == ironoraclaw::config::LlmBackend::NearAi
    ...
    let docker_status = if config.sandbox.enabled {
        let detection = ironoraclaw::sandbox::check_docker().await;
        match detection.status {
            ironoraclaw::sandbox::DockerStatus::Available => {
                tracing::info!("Docker is available");
            }
            ironoraclaw::sandbox::DockerStatus::NotInstalled => {
                tracing::warn!(
                    "Docker is not installed -- sandbox disabled for this session. {}",
                    detection.platform.install_hint()
                );
            }
            ironoraclaw::sandbox::DockerStatus::NotRunning => {
                tracing::warn!(
                    "Docker is installed but not running -- sandbox disabled for this session. {}",
                    detection.platform.start_hint()
                );
            }
            ironoraclaw::sandbox::DockerStatus::Disabled => {}
        }
        detection.status
    } else {
        ironoraclaw::sandbox::DockerStatus::Disabled
    ...
    let job_event_tx: Option<
        tokio::sync::broadcast::Sender<(uuid::Uuid, ironoraclaw::channels::web::types::SseEvent)>,
    ...
    let prompt_queue = Arc::new(tokio::sync::Mutex::new(std::collections::HashMap::<
        uuid::Uuid,
        std::collections::VecDeque<ironoraclaw::orchestrator::api::PendingPrompt>,
    >::new()));
    ...
    let container_job_manager: Option<Arc<ContainerJobManager>> =
        if config.sandbox.enabled && docker_status.is_ok() {
            ...
            let job_config = ContainerJobConfig {
                ...
                claude_code_oauth_token: ironoraclaw::config::ClaudeCodeConfig::extract_oauth_token(),
    ...
    let session_manager =
        Arc::new(ironoraclaw::agent::SessionManager::new().with_hooks(components.hooks.clone()));
    ...
    let mut sse_sender: Option<
        tokio::sync::broadcast::Sender<ironoraclaw::channels::web::types::SseEvent>,
    > = None;
    let mut gateway_state: Option<std::sync::Arc<ironoraclaw::channels::web::server::GatewayState>> =
        None;
    ...
    if config.channels.cli.enabled && cli.message.is_none() {
        let boot_info = ironoraclaw::boot_screen::BootInfo {
        ...
        ironoraclaw::boot_screen::print_boot_screen(&boot_info);
    }
...
async fn run_memory_command(mem_cmd: &ironoraclaw::cli::MemoryCommand) -> anyhow::Result<()> {
    ...
    if config.database.backend == ironoraclaw::config::DatabaseBackend::LibSql
    ...
    let db: Arc<dyn ironoraclaw::db::Database> = ironoraclaw::db::connect_from_config(&config.database)
        .await
        .map_err(|e| anyhow::anyhow!("{}", e))?;

    ironoraclaw::cli::run_memory_command_with_db(mem_cmd.clone(), db, embeddings).await
}
...
async fn run_worker(
    ...
    let config = ironoraclaw::worker::runtime::WorkerConfig {
        job_id,
    ...
    let runtime = ironoraclaw::worker::WorkerRuntime::new(config)
...
async fn run_claude_bridge(
    ...
    let config = ironoraclaw::worker::claude_bridge::ClaudeBridgeConfig {
        job_id,
        orchestrator_url: orchestrator_url.to_string(),
        max_turns,
        model: model.to_string(),
        timeout: std::time::Duration::from_secs(1800),
        allowed_tools: ironoraclaw::config::ClaudeCodeConfig::from_env().allowed_tools,
    };

    let runtime = ironoraclaw::worker::ClaudeBridgeRuntime::new(config)
...
async fn start_tunnel(
    mut config: ironoraclaw::config::Config,
) -> (
    ironoraclaw::config::Config,
    Option<Box<dyn ironoraclaw::tunnel::Tunnel>>,
) {
    ...
    match ironoraclaw::tunnel::create_tunnel(provider_config) {
        Ok(Some(tunnel)) => {
            tracing::info!(
                "Starting {} tunnel on {}:{}...",
                tunnel.name(),
...
struct WasmChannelSetup {
    channels: Vec<(String, Box<dyn ironoraclaw::channels::Channel>)>,
...
async fn setup_wasm_channels(
    config: &ironoraclaw::config::Config,
    secrets_store: &Option<Arc<dyn SecretsStore + Send + Sync>>,
    extension_manager: Option<&Arc<ironoraclaw::extensions::ExtensionManager>>,
    ...
    let mut channels: Vec<(String, Box<dyn ironoraclaw::channels::Channel>)> = Vec::new();
    let mut channel_names: Vec<String> = Vec::new();
...
fn check_onboard_needed() -> Option<&'static str> {
    // Oracle backend is configured via IRONORACLAW_ORACLE_* env vars and
    // doesn't need the onboarding wizard.  Oracle is the default backend,
    // so also skip onboarding when DATABASE_BACKEND is not set at all.
    let is_oracle = std::env::var("DATABASE_BACKEND")
        .map(|v| v.eq_ignore_ascii_case("oracle") || v.eq_ignore_ascii_case("oracledb"))
        .unwrap_or(true); // Default backend is Oracle
    if is_oracle {
        return None;
    }
    ...
    let has_db = std::env::var("DATABASE_URL").is_ok()
        ...
        || ironoraclaw::config::default_libsql_path().exists();
    ...
    if std::env::var("NEARAI_API_KEY").is_err() {
        let session_path = ironoraclaw::llm::session::default_session_path();
...
async fn inject_channel_credentials(
    channel: &Arc<ironoraclaw::channels::wasm::WasmChannel>,
