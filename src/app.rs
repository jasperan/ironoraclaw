    ...
    pub async fn init_database(&mut self) -> Result<(), anyhow::Error> {
        ...
        let db: Arc<dyn Database> = match self.config.database.backend {
            ...
            crate::config::DatabaseBackend::Oracle => {
                ...
                let backend = crate::db::oracle::OracleBackend::new(&self.config.database)
                    .await
                ...
                backend
                    ...
                    .await
                    .map_err(|e| anyhow::anyhow!("{}", e))?;
                tracing::info!("Oracle database connected and migrations applied");
            ...
            _ => {
                anyhow::bail!(
                    "No database backend available. Enable 'oracle', 'postgres', or 'libsql' feature."
                );
            }
        };
