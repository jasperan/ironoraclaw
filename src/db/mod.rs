...
//! operations. Three implementations exist behind feature flags:
//!
//! - `oracle` (default): Oracle AI Database with vector search
//! - `postgres`: Uses `deadpool-postgres` + `tokio-postgres`
...
pub async fn connect_from_config(
    ...
    match config.backend {
        #[cfg(feature = "oracle")]
        crate::config::DatabaseBackend::Oracle => {
            let backend = oracle::OracleBackend::new(config)
                .await
        ...
        _ => Err(DatabaseError::Pool(
            "No database backend available. Enable 'oracle', 'postgres', or 'libsql' feature.".to_string(),
        )),
    }
}
