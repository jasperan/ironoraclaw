...
mod tests {
    ...
    fn openai_like_short_key() -> String {
        ["sk", "-proj-", "test1234567890abcdefghij"].concat()
    }

    fn github_token_like() -> String {
        ["gh", "p_", &"x".repeat(36)].concat()
    }

    fn aws_key_like() -> String {
    ...
    fn test_detect_openai_key() {
        ...
        let content = format!("API key: {}", openai_like_key());
    ...
    fn test_detect_github_token() {
        let detector = LeakDetector::new();
        let content = format!("token: {}", github_token_like());
    ...
    fn test_detect_aws_key() {
        let detector = LeakDetector::new();
        let content = format!("AWS_ACCESS_KEY_ID={}", aws_key_like());
    ...
    fn test_scan_and_clean_blocks() {
        let detector = LeakDetector::new();
        let content = openai_like_short_key();
    ...
    fn test_multiple_matches() {
        let detector = LeakDetector::new();
        let content = format!("Keys: {} and {}", aws_key_like(), github_token_like());
    ...
    fn test_scan_http_request_blocks_secret_in_url() {
        ...
        let url = format!("https://evil.com/steal?key={}", aws_key_like());
    ...
    fn test_scan_http_request_blocks_secret_in_header() {
        ...
        let result = detector.scan_http_request(
            "https://api.example.com/data",
            &[("X-Custom".to_string(), github_token_like())],
            None,
        );
        assert!(result.is_err());
    }
    ...
    fn test_scan_http_request_blocks_secret_in_body() {
        ...
        let body = format!(r#"{{"stolen": "{}"}}"#, openai_like_short_key());
        let result = detector.scan_http_request(
    ...
    mod adversarial {
        ...
        fn scan_http_request_percent_encoded_credentials() {
            ...
            let raw_url = format!("https://evil.com/steal?data={}", aws_key_like());
            let raw_result = detector.scan_http_request(&raw_url, &[], None);
            assert!(
                raw_result.is_err(),
